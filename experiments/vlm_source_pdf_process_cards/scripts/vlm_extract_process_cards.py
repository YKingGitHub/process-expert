#!/usr/bin/env python3
"""Run VLM extraction on rendered source PDF pages."""
...
DEFAULT_ENV_FILE = Path(
    os.environ.get("AI_STACK_ENV_FILE", str(REPO_ROOT / ".env"))
)
EXPERIMENT_ROOT = Path(__file__).resolve().parents[1]
PROMPT_PATH = EXPERIMENT_ROOT / "prompts" / "process_card_vlm_prompt.md"
IMAGE_DIR = EXPERIMENT_ROOT / "fixtures" / "page_images"
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {
                        "type": "text",
                        "text": f"Source PDF page number: {page}\n\n{prompt}",
                    },
                ],
            }
        ],
...
def main() -> int:
    ...
    for page in args.pages:
        ...
        out_path = args.out / f"vlm_page_{page}.json"
        out_path.write_text(
            json.dumps(
