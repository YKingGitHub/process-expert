...
The first framework-driven KB pilot, per
`Projects/process-expert/designs/2026-05-07-01-machining-design-framework/`.

Records are extracted **top-down**: the framework v1.1 fixes the spine
(11 categories, 4 families) and textbook chapters fill the leaves. Cases
(法兰盘 / 定位销) only validate coverage — they no longer drive seed
content.
...
├── scripts/
│   └── rerun_b4_holdout.py         # diff B1+B2 KB vs framework KB on 定位销
└── tests/
...
# Re-run B4 holdout against framework-driven KB
python3 experiments/framework_driven_seed/scripts/rerun_b4_holdout.py
```
...
- **Cohabits with `experiments/capability_seed_expansion/data/candidate_seed.json`**:
  the B1+B2 records are frozen, not migrated. Migration is a separate
  decision once the full framework-driven KB is built.
- **No routing changes**: `experiments/capability_seed_expansion/src/expanded_query.py`
  is left alone. B4 holdout re-eval reads framework_driven_seed directly.
